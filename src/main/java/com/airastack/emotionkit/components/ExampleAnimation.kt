...
private suspend fun RiveStateController.applyState(state: ExampleAnimationType) {
    ...
    when (state) {
        ...
        is ExampleAnimationType.Handsup -> {
            fireState(STATE_MACHINE, "handsup")
        }
        is ExampleAnimationType.Picking -> {
            setState(STATE_MACHINE, AnimationInput.Level.name, 2f)
        }
        is ExampleAnimationType.Fail -> {
            fireState(STATE_MACHINE, "fail")
        }
        is ExampleAnimationType.Success -> {
            fireState(STATE_MACHINE, "success")
        }
    }
}

private const val ART_BOARD = "robot_artboard"
private const val STATE_MACHINE = "robot_login_machine"
...
sealed interface ExampleAnimationType {
    ...
    @Immutable
    data object Handsup: ExampleAnimationType

    @Immutable
    data object Picking: ExampleAnimationType

    @Immutable
    data object Fail: ExampleAnimationType

    @Immutable
    data object Success: ExampleAnimationType
}
